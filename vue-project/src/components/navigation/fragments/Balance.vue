<template>
    <div class="flex items-center gap-2">
        <Button.Native
            ...
            class="py-[4px] px-2 rounded font-medium text-sm text-white"
            :class="{
        ...
        <Button.Native
            v-if="isValidLicenseKey"
            :loading="userDataLoading"
            class="py-[4px] px-2 rounded bg-gray-700 font-medium text-sm text-white mr-4"
            :title="`Remaining SMS Balance ${userData?.sms_balance || 0}tk`"
        >
            SMS: {{ userData?.sms_balance || 0 }}tk
        </Button.Native>
    </div>
</template>
